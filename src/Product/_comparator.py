    ...
    def _comparator(self, collaborator_a_res , collaborator_b_res ):
        ...
        for section_name in sectionproxy:
            ...
            for name, value in parser_proj.items(section_name):
                #first check for the exceptional check removals
                if (      section_name == 'Long Bottom Length' and (
                                    (br_rules.stringBLRO(cmd.flexCmdControlCenter( parser_proj.get('flexCommon','FlexJsonDAOPath'), parser_proj.get('flexCommon','DAOFieldNameKey'),'lscoStyleType',payload=collaborator_a_res, retrievalkey = parser_proj.get('flexCommon','DAOFieldvalue'))) ,
                                    br_rules.stringBLRO(cmd.flexCmdControlCenter( parser_proj.get('flexCommon','FlexJsonDAOPath'), parser_proj.get('flexCommon','DAOFieldNameKey'),'lscoProductSubCat1',payload=collaborator_a_res, retrievalkey = parser_proj.get('flexCommon','DAOFieldvalue'))))
                                      == ('shorts' , 'shorts') or ('skirts', 'skirts'))):
                        if (br_rules.searchBLRO(self.results, section_name )):
                            pass
                        else:
                            self.results.append({'attribute name' : section_name , 'FLEX Compare Key' : 'None', 'PIM Compare Key' : 'None', 'comparison result' : 'Default'})
                elif (compareLock <2 ):
